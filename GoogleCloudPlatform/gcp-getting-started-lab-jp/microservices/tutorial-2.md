...
> 本ハンズオン作成時点では、Workflows はベータ版のためクライアントライブラリーが用意されておらず、Order processor サービスからワークフローを実行する際は、REST API を直接に呼び出しています。この際、API の URL に Project ID が含まれるため、上記のコマンドの最終行では、環境変数 PROJECT_ID を通じて Project ID をコードに受け渡しています。
